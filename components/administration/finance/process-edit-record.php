...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors)) { // If everything's OK.   
            ...
            $query = 'SELECT id FROM financedetails WHERE email=? AND id !=?';
            ...
            if (mysqli_num_rows($result) == 0) { // e-mail does not exist in another record
                ...
                $query = 'UPDATE financedetails 
                    SET firstname=?, lastname=?, gender=?, email=?, address1=?, address2=?, phone=?, experience=?, department=?, hashedpassword=?, user_level=3
                    WHERE id=? 
                    LIMIT 1';
                $q = mysqli_stmt_init($dbcon);
                mysqli_stmt_prepare($q, $query);
                // bind values to SQL Statement
                mysqli_stmt_bind_param($q, 'ssssssssssi', $firstnametrim, $lastnametrim, $gender, $emailtrim, $address1trim, $address2trim, $phonetrim, $experience, $department, $hashedpassword, $id);
                // execute query
                // execute query
                mysqli_stmt_execute($q);
    ...
    $q = mysqli_stmt_init($dbcon);
    $query = "SELECT id, firstname, lastname, gender, email, address1, address2, phone, experience, department
            FROM financedetails
            WHERE id=?";
    mysqli_stmt_prepare($q, $query);
    // bind $id to SQL Statement
        ...
        <div class="row">
            <div class="container mb-md-5">
                <div class="col-md-4 shadow-sm pt-1 m-auto p-md-2">
                    <form action="edit.php" method="POST" onsubmit="return checked();">
                        <div class="form-group text-center">
                            <h3>REGISTRATION</h3>
                        </div>
                        <div class="form-group">
                            <label for="firstname">First Name</label>
                            <input type="text" class="form-control" name="firstname" required placeholder="Enter first Name" value="<?php echo htmlspecialchars($row[1], ENT_QUOTES); ?>">
                        </div>
                        <div class="form-group">
                            <label for="lastname">Last Name</label>
                            <input type="text" class="form-control" name="lastname" required placeholder="Enter last Name" value="<?php echo htmlspecialchars($row[2], ENT_QUOTES); ?>">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" class="form-control" name="email" required placeholder="example@gmail.com" value="<?php echo htmlspecialchars($row[4], ENT_QUOTES); ?>">
                        </div>
                        <div class="form-group">
                            <label for="address1">Address 1</label>
                            <input type="text" class="form-control" name="address1" required placeholder="Enter address 1" value="<?php echo htmlspecialchars($row[5], ENT_QUOTES); ?>">
                        </div>
                        <div class="form-group">
                            <label for="address1">Address 2</label>
                            <input type="text" class="form-control" name="address2" placeholder="Enter address 2" value="<?php echo htmlspecialchars($row[6], ENT_QUOTES); ?>">

                        </div>
                        <div class="form-group">
                            <label for="phonenumber">Phone Number</label>
                            <input type="number" class="form-control" name="phonenumber" placeholder="Enter phone number" required value="<?php echo htmlspecialchars($row[7], ENT_QUOTES); ?>">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-control">
                                ...
                                //make querry
                                $query = "SELECT id ,name FROM financedepartment";
