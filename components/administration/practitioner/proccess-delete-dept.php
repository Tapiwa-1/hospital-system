...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($sure == 'Yes') { // Delete the record.
            ...
            mysqli_stmt_prepare($q, 'DELETE FROM practitionerdepartment WHERE id=? LIMIT 1');
            ...
            if (mysqli_stmt_affected_rows($q) == 1) { // It ran OK
                // Print a message:
                header("Location: administrator-departmentpractitioner.php");
        ...
        $q = mysqli_stmt_init($dbcon);
        $query = "SELECT name
                FROM practitionerdepartment WHERE id=?";
