...
if ((isset($_POST['id'])) && (is_numeric($_POST['id']))) { // Form submission.      #1
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($sure == 'Yes') { // Delete the record.
            ...
            mysqli_stmt_prepare($q, 'DELETE FROM practitionerdetails WHERE id=? LIMIT 1');
            ...
            if (mysqli_stmt_affected_rows($q) == 1) { // It ran OK
                // Print a message:
                header("Location: administrator-viewrecordspractioner.php");
        ...
        $query = "SELECT CONCAT(firstname,' ',lastname) AS name
                FROM practitionerdetails WHERE id=?";
