...
$decisionstring = '<p>Register again here <a href="patient-registration.php">Here</a></p>';
...
if (empty($errors)) { //everything is ok
    try {
        ...
        $query = "SELECT id FROM patientdetails WHERE email = ? ";
        ...
        if (mysqli_num_rows($result) == 0) { //The email address has not been registered
            ...
            //Make a query
            $query = "INSERT INTO patientdetails (id, firstname, lastname, gender, email, address1, address2, phone, hashedpassword, registrationdate,user_level)";
            $query .= "VALUES (' ',?,?,?,?,?,?,?,?,NOW(),4)";
            $q = mysqli_stmt_init($dbcon);
            mysqli_stmt_prepare($q, $query);
